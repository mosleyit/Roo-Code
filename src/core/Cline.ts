...
export class Cline extends EventEmitter<ClineEvents> {
	 ...
	 */
	public async pushToolResult(toolUse: ToolUse, content: ToolResponse): Promise<void> {
		// Make method async
		// Generate the tool description string (logic moved from presentAssistantMessage)
		const toolDescription = async (): Promise<string> => {
		...
		this.userMessageContent.push({
			type: "text",
			text: `${await toolDescription()} Result:`, // Await the async function
		})
